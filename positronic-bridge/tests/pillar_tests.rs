// positronic-bridge/tests/pillar_tests.rs
//
// Integration tests for BioLink (Pillar XII accessibility),
...
#[test]
fn test_waveform_push() {
    let mut buf = WaveformBuffer::new(100);
    buf.push(0.0, 1.0);
    buf.push(0.1, 2.0);
    assert_eq!(buf.len(), 2);
    assert!(!buf.is_empty());
...
#[test]
fn test_waveform_samples_chronological() {
    let mut buf = WaveformBuffer::new(100);
    buf.push(0.0, 1.0);
    buf.push(0.1, 2.0);
    buf.push(0.2, 3.0);
    ...
    assert!((samples[0].0 - 0.0).abs() < f64::EPSILON);
    assert!((samples[2].0 - 0.2).abs() < f64::EPSILON);
}

#[test]
fn test_waveform_ring_buffer_wraps() {
    let mut buf = WaveformBuffer::new(3);
    buf.push(0.0, 1.0);
    buf.push(0.1, 2.0);
    buf.push(0.2, 3.0);
    buf.push(0.3, 4.0); // should evict oldest
    assert_eq!(buf.len(), 3);
    let samples = buf.samples();
    // oldest is now 0.1
    assert!((samples[0].0 - 0.1).abs() < f64::EPSILON);
}
...
fn test_hardware_panel_device_discovered() {
    ...
    assert!(panel.devices.contains_key("COM3"));
    assert!(matches!(
        panel.devices["COM3"].status,
...
fn test_hardware_panel_device_connected() {
    ...
    assert_eq!(panel.devices["COM3"].baud_rate, Some(9600));
    assert!(panel.waveforms.contains_key("COM3"));
}
